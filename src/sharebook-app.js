...
import './sharebook-home.js';
import { afterNextRender } from '@polymer/polymer/lib/utils/render-status.js';
import { timeOut } from '@polymer/polymer/lib/utils/async.js';
import { Debouncer } from '@polymer/polymer/lib/utils/debounce.js';

// performance logging
window.performance && performance.mark && performance.mark('sharebook-app - before register');

class SharebookApp extends PolymerElement {
  static get template() {
    return html`
    <style>
      ...
      sharebook-tabs, sharebook-tab {
        --sharebook-tab-overlay: {
    ...
    <sharebook-analytics key="UA-148606195-1"></sharebook-analytics>
    <!--
    ...
    <!--
    sharebook-category-data provides the list of categories.
    -->
    <sharebook-category-data categories="{{categories}}"></sharebook-category-data>

    <!--
    sharebook-cart-data maintains the state of the user's shopping cart (in localstorage) and
      calculates the total amount.
    -->
    <sharebook-cart-data id="cart" cart="{{cart}}" num-items="{{numItems}}" total="{{total}}"></sharebook-cart-data>
    ...
    <app-header role="navigation" id="header" effects="waterfall" condenses="" reveals="">
      <app-toolbar>
        ...
        <div class="logo" main-title=""><a href="/" aria-label="Sharebook Home">Sharebook</a></div>
      ...
      <div id="tabContainer" sticky\$="[[_shouldShowTabs]]" hidden\$="[[!_shouldShowTabs]]">
        <dom-if if="[[_shouldRenderTabs]]">
          <template>
            <sharebook-tabs selected="[[categoryName]]" attr-for-selected="name">
              <dom-repeat items="[[categories]]" as="category" initial-count="4">
                <template>
                <sharebook-tab name="[[category.name]]">
                  <a href="/list/[[category.name]]">[[category.title]]</a>
                </sharebook-tab>
                </template>
              </dom-repeat>
              <sharebook-tab name="new">
                <a href="/new">New</a>
              </sharebook-tab>
            </sharebook-tabs>
          </template>
    ...
    <iron-pages role="main" selected="[[page]]" attr-for-selected="name" selected-attribute="visible" fallback-selection="404">
      <!-- home view -->
      <sharebook-home name="home" categories="[[categories]]"></sharebook-home>
      <!-- list view of items in a category -->
      <sharebook-list name="list" route="[[subroute]]" offline="[[offline]]"></sharebook-list>
      <!-- detail view of one item -->
      <sharebook-detail name="detail" route="[[subroute]]" offline="[[offline]]"></sharebook-detail>
      <!-- detail view of insert item -->
      <sharebook-new name="new" route="[[route]]" offline="[[offline]]"></sharebook-new>
      <!-- cart view -->
      <sharebook-cart name="cart" cart="[[cart]]" total="[[total]]"></sharebook-cart>
      <!-- checkout view -->
      <sharebook-checkout name="checkout" cart="[[cart]]" total="[[total]]" route="{{subroute}}"></sharebook-checkout>

      <sharebook-404-warning name="404"></sharebook-404-warning>
    </iron-pages>

    <footer>
      <a href="https://www.polymer-project.org/3.0/toolbox/">Made with Polymer</a>
      <div class="demo-label">Sharebook</div>
    </footer>
...
`;
  ...
  static get is() { return 'sharebook-app'; }
  ...
  constructor() {
    super();
    window.performance && performance.mark && performance.mark('sharebook-app.created');
  }
  ...
  _pageChanged(page, oldPage) {
    if (page != null) {
      ...
      switch (page) {
        case 'list':
          import('./sharebook-list.js').then(cb);
          break;
        case 'detail':
          import('./sharebook-detail.js').then(cb);
          break;
        case 'cart':
          import('./sharebook-cart.js').then(cb);
          break;
        case 'checkout':
          import('./sharebook-checkout.js').then(cb);
          break;
                case 'checkout':
          import('./sharebook-checkout.js').then(cb);
          break;
        case 'new':
            import('./sharebook-new.js').then(cb);
            break;
        default:
  ...
  _notifyNetworkStatus() {
    ...
    if (this.offline || (!this.offline && oldOffline === true)) {
      if (!this._networkSnackbar) {
        this._networkSnackbar = document.createElement('sharebook-snackbar');
        this.root.appendChild(this._networkSnackbar);
      }
      this._networkSnackbar.innerHTML = this.offline ?
          'Est&aacute;s sin conexi&oacute;n' : 'Est&aacute;s en l&iacute;nea';
  ...
  _onChangeSection(event) {
    ...
    if (detail.title) {
      document.title = detail.title + ' - SHAREBOOK';
  ...
  _onAddCartItem(event) {
    if (!this._cartModal) {
      this._cartModal = document.createElement('sharebook-cart-modal');
      this.root.appendChild(this._cartModal);
    }
    this.$.cart.addItem(event.detail);
    this._cartModal.open();
    this._announce('Art&iacute;culo agregado al carrito.');
  }
  ...
  _onSetCartItem(event) {
    ...
    if (detail.quantity === 0) {
      this._announce('Art&iacute;culo eliminado.');
    } else {
      this._announce('La cantidad ha cambiado a ' + detail.quantity);
    }
  }

  _onClearCart() {
    this.$.cart.clearCart();
    this._announce('Carrito eliminado');
  }
  ...
  _computeShouldShowTabs(page, smallScreen) {
    return (page === 'home' || page === 'list' || page === 'detail' || page === 'new') && !smallScreen;
  }
...
customElements.define(SharebookApp.is, SharebookApp);
